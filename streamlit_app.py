...
# --- MOTEUR D'INTELLIGENCE DE QUARTIER (V15.2) ---
...
with tab1:
    ...
    # --- BARRE LATÉRALE (FINANCEMENT COMPLET) ---
    st.sidebar.header("🏦 Stratégie de Financement")
    apport = st.sidebar.number_input("Apport personnel (€)", value=0, help="Somme injectée par la SCI.")
    ...
    # --- PARTIE MANQUANTE : ANALYSE D'OPPORTUNITÉ ---
    st.divider()
    ...
    with m2:
        if st.button("🔍 Lancer le Diagnostic Sécurité & Mixité Sociale"):
            with st.spinner("Analyse des bases de données..."):
                time.sleep(1)
    ...
    if st.button("🚀 Enregistrer et Partager avec la famille", use_container_width=True):
        if gsheet:
            ...
            st.success("C'est enregistré ! Allez dans le comparateur.")
            time.sleep(1)
            st.rerun()
